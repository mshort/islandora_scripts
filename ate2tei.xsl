<!-- xhtml2tei.xsl base from http://tei.oucs.ox.ac.uk/Talks/2011-05-26-lyon/Work/release/tools2/xhtml2tei.xsl -->
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="h:html">
        ...
        <xsl:variable name="title">
            <xsl:value-of select="h:head/h:meta[@name='DC.title']/@content"/>
        </xsl:variable>
        <xsl:variable name="author">
            <xsl:choose>
                <xsl:when test="h:head/h:meta[@name='DC.creator']">
                    <xsl:value-of select="h:head/h:meta[@name='DC.creator']/@content"/>
                </xsl:when>
        ...
        <xsl:variable name="date">
            <xsl:choose>
                <xsl:when test="h:head/h:meta[@name='DC.date']">
                    <xsl:value-of select="h:head/h:meta[@name='DC.date']/@content"/>
                </xsl:when>
                <xsl:when test="h:body/h:address">
                    <xsl:variable name="add">
                        <xsl:value-of select="h:body/h:address"/>
        ...
        <TEI.2>
            <teiHeader>
                <fileDesc>
                    ...
                    <publicationStmt>
                        <authority>Northern Illinois University</authority>

                    </publicationStmt>
                    <sourceDesc>
                        <p>This is the master version of an original document, created by converting
                            an HTML file using ATE to TEI XML.</p>
                    </sourceDesc>
                </fileDesc>
    ...
    <xsl:template match="h:body">
        <body>
            ...
            <xsl:variable name="starter">
                <xsl:value-of
                    select="substring(name(//*[(name()='h1' or name()='h2' or name()='h3' or name()='h4') and name(..)!= 'td'][1]),2)"
                />
            </xsl:variable>
            <xsl:variable name="ender">
                <xsl:value-of
                    select="substring(name(//*[(name()='h1' or name()='h2' or name()='h3' or name()='h4') and name(..)!= 'td'][last()]),2)"
                />
            </xsl:variable>
...
got to find a way to set a trigger for closing div if no headers -->
    ...
    <xsl:template match="h:h1|h:h2|h:h3|h:h4">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:if
                    test="preceding-sibling::h:h1 or preceding-sibling::h:h2 or preceding-sibling::h:h3 or preceding-sibling::h:h4">
                    <xsl:variable name="previous">
                        <xsl:value-of
                            select="substring(name(preceding-sibling::*[name()='h1' or name()='h2' or name()='h3' or name()='h4'][1]),2)"
                        />
                    </xsl:variable>
...
-->
    ...
    <!-- rules for handling ATE figures -->

    <xsl:template match="h:p/h:figure">
        <xsl:choose>
            <xsl:when test="@inline='n'">
                <figure>
    ...
    <!-- rules for handling ATE page numbers -->

    <xsl:template match="h:page">
        <pb>
            <xsl:attribute name="n">
    ...
    <xsl:template match="h:li/h:p">
        <xsl:choose>
            <xsl:when test="count(parent::h:li/h:p)&gt;1">
                <p>
                    <xsl:apply-templates/>
                </p>
            </xsl:when>
    ...
    <xsl:template match="h:td/h:p">
        <xsl:choose>
            <xsl:when test="count(parent::h:td/h:p)&gt;1">
                <p>
                    <xsl:apply-templates/>
                </p>
            </xsl:when>
    ...
    <xsl:template match="h:tr">
        <row>
         ...
         <xsl:template match="h:p/h:table">
        <table>
            <xsl:apply-templates/>
        </table>
    </xsl:template>
